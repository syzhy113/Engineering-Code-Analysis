...
def get_column_names(
    ...
    # 读取 Excel 文件的第一个工作表
    df = pd.read_excel(filename, sheet_name=0, header=None)  # sheet_name=0 表示第一个工作表
...
def get_first_n_rows(
    ...
    # 读取 Excel 文件的第一个工作表
    df = pd.read_excel(filename, sheet_name=0, engine='openpyxl')  # sheet_name=0 表示第一个工作表
...
if __name__ == '__main__':
    file_name = '../data/labels.xlsx'
    get_first_n_rows(file_name, 3)
